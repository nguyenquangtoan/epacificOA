...
function ai_viewBuddle(id_div, type, image, title){
    ...
    var html = '';
    var top_login_image = `
        <button class="top_logout_button" onclick="location.href = '` + sign_in_url + `'">    
                Sign In
        </button>
    `;
    ...
    if(token && token != 'undefined'){
        ...
        top_login_image = `
            <div class="top_login_image" onclick="ai_openAccountBox();">
                <div>
                    <img id="top_account_icon" src="` + user_avatar + `" style="width:36px; height:36px;padding:2px;">
                </div>
            </div>`;
        arvatar_auto_gen = 1;
        if(arvatar_auto_gen == 1){
                ...
                top_login_image = `
                    <div class="top_login_image" onclick="ai_openAccountBox();">
                        <div>
                            <img id="top_account_icon" src="` + user_avatar + `" style="width:36px; height:36px;padding:2px;border-radius:20px;">
                        </div>
                    </div>`;
            }
    }

    html = top_login_image + `
                <div class="top_login_image">
                    <div style="">       
                        <img id="top_menu_icon" src="` + menu_logo + `" style="width:24px; height:24px;padding:8px;" onclick="ai_openMenuBox();">
                    </div>
                </div>
...
function ai_openMenuBox(){
    ...
    if(box_display != 'block' || ai_box_login != 'MenuBox'){
        ...
        var html = '';
        html = `
        <iframe id="top_component_iframe" src="` + top_component_url + `?protocol=` + location.protocol + `&host=`+ location.host +`&port=` + location.port + `" title="description" allowfullscreen
...
function ai_openAccountBox(){
    ...
    if(box_display != 'block'  || ai_box_login != 'AccountBox'){
        ...
        var html = '';
        html = `
        <iframe id="top_account_iframe" src="` + top_account_url + `?protocol=` + location.protocol + `&host=`+ location.host +`&port=` + location.port + `" title="description" allowfullscreen
            style="background:transparent;margin:10px;"
        ></iframe>
        `;
        ai_chat_box.innerHTML = html;
...
function ai_initLogin(avatars, project, project_chatbot){
    ...
    ai_viewBuddle('chat-overlay', project_chatbot.widget_bubble_type, avatars.buddle_avatar, project_chatbot.widget_bubble_title);
...
function recAccountBox(event) {  
    ...
    if (event.origin != "http" + ssl + "://" + domain_name) { return; }  
...
function closeAccountBoxFromOtherClick(event){
    if(event.target.id != 'top_menu_icon' && event.target.id !='top_account_icon'){
        ai_chat_box.style.display = 'none';
        ai_box_login = '';
    }
}
...
function sigout() {
    ...
    fetch('https://identity-stg.epacific.net/realms/epacific/protocol/openid-connect/logout', {
      method: 'POST',
      headers: {
    ...
    .then (function() {
            ...
            setCrossSubdomainCookie(refresh_token_name,refresh_token, -30, main_domain_name);
  ...
  function refreshToken(){
    ...
    fetch('https://identity-stg.epacific.net/realms/epacific/protocol/openid-connect/token', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/x-www-form-urlencoded'
      },
    ...
    .then(data => {
        ...
        setCrossSubdomainCookie(token_name,data.access_token, 30, main_domain_name);
        token = data.access_token;
        setCrossSubdomainCookie(refresh_token_name,data.refresh_token, 30, main_domain_name);
